...
class DingtalkChatbot(object):
    ...
    def send_link(self, title, text, message_url, pic_url=''):
        ...
        """
        if all(map(is_not_null, [title, text, message_url])):
            data = {
                "msgtype": "link",
                "link": {
                    "text": text,
    ...
    def send_markdown(self, title, text, is_at_all=False, at_mobiles=[], at_dingtalk_ids=[], is_auto_at=True):
        ...
        :return: 返回消息发送结果
        """
        if all(map(is_not_null, [title, text])):
...
class ActionCard(object):
    ...
    def get_data(self):
        ...
        """
        if all(map(is_not_null, [self.title, self.text])) and len(self.btns):
            if len(self.btns) == 1:
                # 整体跳转ActionCard类型
                data = {
                    "msgtype": "actionCard",
...
class FeedLink(object):
    ...
    def get_data(self):
        ...
        """
        if all(map(is_not_null, [self.title, self.message_url, self.pic_url])):
            data = {
                "title": self.title,
...
class CardItem(object):
    ...
    def get_data(self):
        ...
        @return: 子控件的数据
        """
        if all(map(is_not_null, [self.title, self.url, self.pic_url])):
            # FeedCard类型
            data = {
                "title": self.title,
                "messageURL": self.url,
            ...
            return data
        elif all(map(is_not_null, [self.title, self.url])):
            # ActionCard类型
            data = {
                "title": self.title,
                "actionURL": self.url
