...
def set_content_type(method):
    """
    设置请求类型
    :param method: 数据类型 --> json格式 from表单
    :return:
    """
    if method == "json":
...
def set_headers(method,other_info):
    """
    设置头部信息
    :param method:
    :return:
    """
    headers = set_content_type(method)
    for add_key in other_info.keys():
        headers.setdefault(add_key, other_info.get(add_key)) # 若存在则不覆盖
    return headers

if __name__ == '__main__':
    logger.info(random_agent())
    logger.info(set_content_type("json"))
    jsonHeaders = {
        "Host": "yuyanqing.cn:9010",
    ...
    logger.info(set_headers("json", jsonHeaders))