...
def fix_data():
    fix_time.clear()
    fix_id.clear()
    for fix in SyncRequest.response_time:
        fix_time.append(format(fix, '.2f'))
    for id in SyncRequest.ids:
        fix_id.append(id)
...
def show_pie(id):
    attr = ["成功（含熔断）", "失败", "限流"]
    counts = [len(SyncRequest.success), len(SyncRequest.fail), SyncRequest.limit]
...
def show_bar(id):
    ...
    bar.render('单压测试{}.html'.format(id + 1))
